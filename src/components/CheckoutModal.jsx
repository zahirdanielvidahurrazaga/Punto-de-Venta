...
import { CreditCard, Banknote, Building, X, ArrowRight, Wallet, SplitSquareHorizontal, CheckCircle } from 'lucide-react';
...
export default function CheckoutModal({ total, onClose, onComplete }) {
  ...
  const [mode, setMode] = useState('quick'); // 'quick', 'cash', 'split'
  const [cashReceived, setCashReceived] = useState('');
  const cashInputRef = useRef(null);

  useEffect(() => {
    if (mode === 'cash' && cashInputRef.current) {
  ...
  const handleQuickPay = (method) => {
    if (method === 'efectivo') {
      // Abrir flujo de efectivo para ingresar cantidad recibida
      setMode('cash');
      return;
    }
    onComplete({
    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-200 flex flex-col max-h-[95vh]">
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-3 gap-3">
                <button onClick={() => handleQuickPay('efectivo')} className="flex flex-col items-center p-4 bg-slate-50 hover:bg-amber-50 text-slate-700 hover:text-amber-700 rounded-2xl border border-slate-200 hover:border-amber-300 transition-all shadow-sm hover:shadow-md group">
                  ...
                  <span className="text-xs lg:text-sm font-bold text-center leading-tight">Efectivo</span>
                </button>
          ...
          {mode === 'cash' && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
              ...
              <div className="bg-amber-50 p-5 rounded-2xl border border-amber-200">
                <label className="text-xs font-bold text-amber-700 uppercase tracking-wider block mb-3">
                ...
                <div className="relative flex items-center">
                  ...
                  <input 
                    ...
                    className="w-full bg-transparent outline-none text-4xl font-black text-slate-800 placeholder:text-slate-300" 
                  />
                </div>
              </div>

              {/* Cálculo de cambio */}
              <div className="bg-slate-800 p-5 rounded-2xl shadow-inner space-y-3">
                ...
                <div className="flex justify-between text-slate-300 font-medium text-lg">
                  <span>Recibido:</span>
                  <span className={cashReceivedNum > 0 ? "text-white font-bold" : "text-slate-500"}>${cashReceivedNum.toFixed(2)}</span>
                </div>
                {cashIsValid && (
                  <div className="flex justify-between text-amber-400 font-black text-2xl pt-3 border-t border-slate-700">
                    <span>Cambio:</span>
                    <span>${cashChange.toFixed(2)}</span>
                  </div>
                )}
                {!cashIsValid && cashReceivedNum > 0 && (
                  <div className="flex justify-between text-orange-400 font-bold text-lg pt-3 border-t border-slate-700">
                    <span>Faltan:</span>
              ...
              <button 
                ...
                className="w-full py-3 bg-slate-100 text-slate-600 hover:bg-slate-200 rounded-xl font-bold text-sm transition-colors"
              >
          ...
          {mode === 'split' && (
            <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-300">
              <div className="flex justify-between items-center mb-2">
                ...
                <button onClick={() => setMode('quick')} className="text-sm text-primary-600 font-bold hover:underline">
                  Volver
                </button>
              </div>
        ...
        <div className="p-4 md:p-6 bg-slate-50 border-t border-slate-100 flex gap-3 md:gap-4 shrink-0">
          <button onClick={() => mode === 'quick' ? onClose() : setMode('quick')} className="flex-1 py-3 md:py-4 text-slate-600 font-bold rounded-xl hover:bg-slate-200 transition-colors">
          ...
          {mode === 'cash' && (
            <button 
              ...
              className="flex-[2] bg-slate-900 hover:bg-slate-800 disabled:bg-slate-300 disabled:cursor-not-allowed text-white py-3 md:py-4 rounded-xl font-bold text-lg transition-all flex items-center justify-center gap-2 shadow-lg"
            >
